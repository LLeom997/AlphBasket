...
import { runSimulation } from "../analytics/simulationEngine";
import { getTodayDate, getDateYearsAgo } from "../../utils/dateUtils";
import { NSE_MASTER_LIST_URL, DEFAULT_TICKERS } from "../../utils/constants";
...
export async function getNseMasterList(): Promise<any[]> {
    ...
    try {
        const response = await fetch(NSE_MASTER_LIST_URL);
...
async function ensureBenchmark() {
    ...
    try {
        ...
        const from = getDateYearsAgo(6); // Fetch 6 years for 5Y CAGR
...
/**
 * Initializes default stock data
 */
export async function initDefaultStocks() {
    await ensureStockHistory(DEFAULT_TICKERS);
}
