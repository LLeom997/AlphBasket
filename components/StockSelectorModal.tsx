...
import { Search, X, Check, Filter, RefreshCcw, ChevronDown, Loader2, ArrowUpNarrowWide, ArrowDownWideNarrow } from 'lucide-react';
...
const StockCard: React.FC<StockCardProps> = ({ item, isSelected, onToggle }) => {
  ...
  return (
    <div 
      ...
      className={`cursor-pointer rounded-2xl border transition-all flex flex-col group relative overflow-hidden bg-white min-h-[120px] ${isSelected ? 'border-indigo-500 ring-4 ring-indigo-500/5 bg-indigo-50/10' : 'border-slate-100 hover:border-indigo-200'}`}
    >
      <div className="p-3 flex flex-col h-full">
        <div className="flex justify-between items-start mb-2">
            ...
            {loading && <Loader2 size={8} className="animate-spin text-indigo-400" />}
        </div>

        <div className="grid grid-cols-2 gap-1.5 mt-auto">
            <div className="flex flex-col">
                <span className="text-[6px] font-black text-slate-400 uppercase tracking-widest">1Y Growth</span>
                <span className={`text-[9px] font-black ${syncedData?.returns.oneYear && syncedData.returns.oneYear > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
            ...
            <div className="flex flex-col text-right">
                ...
                <span className={`text-[9px] font-black ${syncedData?.returns.fiveYear && syncedData.returns.fiveYear > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
        ...
        <div className="h-4 w-full mt-2 -mx-3 w-[calc(100%+24px)] opacity-30">
            {syncedData && sparkData.length > 0 && (
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={sparkData}><Line type="monotone" dataKey="value" stroke="#4f46e5" strokeWidth={1.5} dot={false} isAnimationActive={false}/></LineChart>
                </ResponsiveContainer>
            )}
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <div className="bg-white border border-slate-200 rounded-[32px] w-full max-w-6xl h-[85vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
          <div>
            <h2 className="text-lg font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">Asset Explorer <span className="text-[8px] px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full">{loadingMaster ? 'Syncing' : `${masterList.length} Symbols`}</span></h2>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-rose-500 p-2 rounded-lg transition-all"><X size={20} /></button>
        </div>
        ...
        <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 shrink-0 flex flex-col sm:flex-row gap-2">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search Assets..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-[10px] font-black outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all" />
            </div>
            ...
            <div className="flex bg-white p-0.5 rounded-xl border border-slate-200">
                {['symbol', '1Y', '5Y'].map(k => (
                    <button key={k} onClick={() => { if (sortKey === k) setSortOrder(prev => prev === 'asc' ? 'desc' : 'asc'); else { setSortKey(k as SortKey); setSortOrder('desc'); } }} className={`px-3 py-1.5 rounded-lg text-[7px] font-black uppercase tracking-widest ${sortKey === k ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>
                        {k === 'symbol' ? 'Ticker' : k}
                    </button>
                ))}
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/20 custom-scrollbar">
          {loadingMaster ? <div className="flex items-center justify-center h-full"><RefreshCcw className="animate-spin text-indigo-600" size={32} /></div> : (
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-white flex justify-between items-center shrink-0">
            ...
            <div className="flex gap-2">
                ...
                <button onClick={() => { onAdd(Array.from(selectedTickers)); setSelectedTickers(new Set()); onClose(); }} disabled={selectedTickers.size === 0} className="px-6 py-2 rounded-xl text-[9px] font-black bg-indigo-600 text-white disabled:opacity-50 uppercase tracking-widest shadow-lg shadow-indigo-100">Assemble Basket</button>
            </div>
        </div>
      </div>
    </div>
