    ...
    return (
        <div className="space-y-4 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-brand-teal/5 border border-brand-teal/10 p-4 rounded-xl shadow-sm">
                    <p className="text-[8px] font-bold uppercase tracking-widest text-brand-teal opacity-70 mb-1">Effective Deployed</p>
                    <p className="text-lg font-bold text-slate-900">{formatMoney(totalActualAmount)}</p>
                    <div className="mt-1 flex items-center justify-between text-[8px] font-semibold text-brand-teal">
                ...
                <div className="bg-brand-green/5 border border-brand-green/10 p-4 rounded-xl shadow-sm">
                    <p className="text-[8px] font-bold uppercase tracking-widest text-brand-green opacity-70 mb-1">Allocation Drift</p>
                    <p className="text-lg font-bold text-brand-green">{formatPct(Math.abs(totalTargetWeight - totalActualWeight))}</p>
                    <Scale size={12} className="mt-1 text-brand-green/40" />
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                    <h3 className="text-slate-800 font-bold text-[10px] uppercase tracking-wider flex items-center gap-2">
                        <PieChart size={14} className="text-brand-teal" />
                        Strategy Execution Plan
                    </h3>
                    <span className="text-[8px] text-brand-teal font-bold uppercase tracking-widest px-2 py-0.5 bg-white rounded border border-slate-200">Real-Time Sync</span>
                </div>
                            ...
                            <tr>
                                <td className="px-5 py-4">
                                    <div className="flex items-center gap-2">
                                        <Sigma size={12} className="text-brand-teal" />
                                ...
                                <td className="px-5 py-4 text-right">
                                    <div className="text-xs font-bold text-brand-teal">{formatMoney(totalActualAmount)}</div>
                                </td>
                            </tr>
                        </tfoot>
                    </table>
