  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm hover:border-indigo-300 transition-all group">
              <div className="flex items-center gap-3 mb-3">
                <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg group-hover:scale-110 transition-transform"><Sparkles size={16} /></div>
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm hover:border-emerald-300 transition-all group">
              <div className="flex items-center gap-3 mb-3">
                <div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg group-hover:scale-110 transition-transform"><Landmark size={16} /></div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em]">Passive Payout</span>
              </div>
              <p className="text-3xl font-bold text-emerald-600 tracking-tight">{formatINR(passiveMonthly)}</p>
              <p className="text-[9px] font-semibold text-slate-500 mt-2 uppercase">Est. Monthly Dividend (5% Ann.)</p>
          </div>
          
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm hover:border-amber-300 transition-all group">
              <div className="flex items-center gap-3 mb-3">
                <div className="p-2 bg-amber-50 text-amber-600 rounded-lg group-hover:scale-110 transition-transform"><Timer size={16} /></div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em]">Doubling Time</span>
              </div>
              <p className="text-3xl font-bold text-amber-600 tracking-tight">{doubleYears} Years</p>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
              ...
              <div className="flex items-center gap-6 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs font-bold text-emerald-600">+{formatINR(terminalWealth - totalInvested)}</p>
                  </div>
                  <div className="w-px h-6 bg-slate-200"></div>
                  <div className="text-center">
                    <p className="text-[8px] font-bold text-slate-400 uppercase mb-1">Strategy CAGR</p>
                    <p className="text-xs font-bold text-indigo-600">{formatPct(simulation.metrics.cagr)}</p>
                  </div>
              </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              <div className="lg:col-span-4 space-y-10">
                  <div className="space-y-5">
                      <div className="flex justify-between items-center px-1">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                            <IndianRupee size={12} className="text-indigo-600" /> Monthly Contribution
                        </label>
                  ...
                  <div className="space-y-5">
                      <div className="flex justify-between items-center px-1">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                            <Target size={12} className="text-indigo-600" /> Time Horizon
                        </label>
                  ...
                  <div className="bg-slate-900 p-6 rounded-2xl shadow-xl shadow-slate-200 relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-2 relative z-10">
                          ...
                          <span className="text-[10px] font-semibold text-emerald-400 uppercase">Projection Baseline Verified</span>
                      </div>
              ...
              <div className="lg:col-span-8 flex flex-col">
                  <div className="flex items-center justify-between mb-8 px-2">
                      <h4 className="text-[10px] font-bold text-slate-900 uppercase tracking-widest flex items-center gap-2">
                        <LineChart size={14} className="text-indigo-600" /> Probability & Growth Corridor
                      </h4>
                  ...
                  <div className="flex-1 h-[360px]">
                      <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                              ...
                              <Area type="monotone" name="Bullish Peak" dataKey="bull" stroke="#10b981" strokeWidth={1} strokeDasharray="4 4" fill="url(#bullGrad)" />
                              <Area type="monotone" name="Net Principal" dataKey="invested" stroke="#cbd5e1" strokeWidth={1} fill="#f8fafc" />
                              <Area type="monotone" name="Expected Growth" dataKey="corpus" stroke="#4f46e5" strokeWidth={3} fill="url(#wealthGrad)" animationDuration={1000} />
                              <Area type="monotone" name="Bearish Floor" dataKey="bear" stroke="#rose-300" strokeWidth={1} strokeDasharray="4 4" fill="transparent" />
                          </AreaChart>
                      </ResponsiveContainer>
                  </div>
                  
                  <div className="mt-6 p-5 bg-indigo-50 border border-indigo-100 rounded-xl flex items-start gap-4">
