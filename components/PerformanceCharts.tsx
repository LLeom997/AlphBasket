...
const COLORS = ['#5acec9', '#8dd67a', '#fa9f42', '#f3533b', '#64748b', '#94a3b8', '#cbd5e1'];
...
const Candlestick = (props: any) => {
  ...
  const color = isUp ? '#8dd67a' : '#f3533b';
    ...
    return (
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl shadow-2xl text-white min-w-[180px]">
        <div className="flex justify-between items-center mb-2 pb-2 border-b border-slate-800">
          ...
          <Activity size={10} className="text-brand-teal" />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between gap-4">
            ...
            <span className="text-[10px] font-black text-brand-green">₹{data.high?.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
          </div>
          <div className="flex justify-between gap-4">
            <span className="text-[8px] font-bold text-slate-500 uppercase">Low</span>
            <span className="text-[10px] font-black text-brand-red">₹{data.low?.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
          </div>
          <div className="flex justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
              <button
                ...
                className={`p-1.5 rounded-md transition-all ${chartType === 'area' ? 'bg-white text-brand-teal shadow-sm' : 'text-slate-400'}`}
              >
                <Layout size={14} />
              </button>
              <button
                onClick={() => setChartType('candlestick')}
                className={`p-1.5 rounded-md transition-all ${chartType === 'candlestick' ? 'bg-white text-brand-teal shadow-sm' : 'text-slate-400'}`}
              >
          ...
          <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-200">
            {(['1Y', '3Y', '5Y', 'ALL'] as Period[]).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-3 py-1 text-[9px] rounded-md font-bold transition-all ${period === p ? 'bg-brand-teal text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'
                  }`}
              >
                {p}
              </button>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'area' ? (
              <AreaChart data={filteredData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="close" stroke="#5acec9" strokeWidth={2.5} fillOpacity={1} fill="url(#colorMain)" animationDuration={1000} />
              </AreaChart>
            ) : (
          ...
          <div className="flex flex-wrap gap-2">
            {comparisonSeries?.map((s, idx) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider border transition-all flex items-center gap-2 ${activeComparisons.includes(s.ticker) ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-400 border-slate-100 hover:border-slate-300'
                  }`}
              >
