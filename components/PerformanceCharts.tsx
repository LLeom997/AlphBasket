...
import { Calendar, X, Loader2, Filter, TrendingUp, Award, Zap, History } from 'lucide-react';
...
const PerformanceCharts: React.FC<PerformanceChartsProps> = ({ history, buyAndHoldHistory, drawdownData, comparisonSeries }) => {
  ...
  const toggleAll = () => {
    if (activeComparisons.length === (comparisonSeries?.length || 0)) setActiveComparisons([]);
    else setActiveComparisons(comparisonSeries?.map(s => s.ticker) || []);
  };
  ...
  const calculateCAGRForWindow = (data: any[]) => {
      ...
      return years > 0.1 ? Math.pow(endVal / startVal, 1 / years) - 1 : (endVal - startVal) / startVal;
  };
  ...
  const filteredData = useMemo(() => {
    ...
    if (data.length > 0) {
        ...
        const basketBase = basePoint.close;
        return data.map(point => {
            const normalized: any = { ...point };
            activeComparisons.forEach(ticker => {
                if (point[ticker] !== undefined && basePoint[ticker] !== undefined) {
                    normalized[ticker] = (point[ticker] / basePoint[ticker]) * basketBase;
                }
            });
            return normalized;
        });
      ...
      <div className="bg-white p-4 rounded-[24px] border border-slate-200 shadow-sm space-y-3">
        ...
        <div className="h-[200px] sm:h-[300px] w-full relative">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={filteredData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
              ...
              <YAxis stroke="#cbd5e1" tickFormatter={formatCurrency} tick={{fontSize: 7, fontWeight: 700}} domain={['auto', 'auto']} width={65} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', fontSize: '8px', fontWeight: 900 }} formatter={(value: number) => [formatCurrency(value), 'Value']} labelFormatter={(label) => new Date(label).toDateString()} />
        ...
        <div className="pt-3 border-t border-slate-50">
            <div className="flex items-center justify-between mb-2">
                <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><Filter size={10}/> Track Assets</span>
                <button onClick={toggleAll} className="text-[7px] font-black text-indigo-600 uppercase hover:underline">
